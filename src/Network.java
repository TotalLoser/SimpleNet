...
/**
 * Class to represent a network of <b> Neuron </b> objects which, together, learn to solve a 
 * given problem
...
public class Network 
{
	 ...
	 * @param tneurons number of neurons in a layer
	 * @param tnumPossibles number of possible answers
	 * @param titerations number of hidden layers
	 * @param tnumInputs number of inputs
	 */
	public Network(int tneurons, int titerations, int tnumInputs, int tnumPossibles)
	{
		...
		neuronArray = new Neuron[iterations][neurons];
		// populate the array with neurons with random wieght (load function coming soon)
	...
	/**
	 * Network attempts to solve a problem and corrects nessecary wieghts if it fails
	 * @param expression the expression intended to be solved
	 * @return true if the answer the network was at least 95% accurate to the actual answer
	...
	public boolean learn(String expression) throws IllegalArgumentException
	{
		...
		// parse all the elements from components and put them into parsedComponents
		 ...
		 */
		int	answer;
		// if the expression asking to be learned is outside the scope of this neural net throw exception
		if(components.length != numInputs)
			throw new IllegalArgumentException("experession had " + components.length + " components, expected " + numInputs);
		...
		for(Neuron ne : this.outputs)
		{
			...
			/* if the neuron with the answer doesn't fire close to full or the non-answer neurons
			*  dont fire close to empty return false
			*/
	...
	/**
	 * recursivley fire all neurons in network with each output feeding into the input of others
	 * @param inputs the inputs that casue the first neurons to fire
	 */
	 ...
	 * @param inputs the inputs that cause the first neurons to fire
	 * @param index the iteration currently being fired
	 */
		...
		for(int i = 0; i < neurons; i++)
		{
			if(neuronArray[index][i].getState() == 0)
			{
				if(index == 0)
				{
					...
					//System.out.println("set neuron " + i + " to " + neuronArray[index][i].getState());
					//pw.println("set neuron " + i + " to " + neuronArray[index][i].getState());
				}
				else
				{
...
//			}
	 ...
	 * @param netfilename name of file to store network data
	 * @param neurfilename name of file to store neuron data
